...
The same fix pattern applies to any other on-demand tool that needs booted service state. `ServerStorage.OMF_RunBalanceDigest` (an aggregate QA report over `RunReviewService.history()`, batching a TypeSafe call across many runs rather than one) is invoked identically. It additionally **yields** — it makes a live HTTP call whenever there is enough history to ask about — so treat an `Invoke()` on it exactly like the long-running-call case below: `task.spawn` it, never block on it directly.
...
**A spec that runs after Phase10_Boss inherits a player standing on ground that is about to vanish.** Its teardown calls `FloorService.endRun()` directly on a still-alive character (never a real gameplay path — a real death zeroes `Humanoid.Health` and lets `spawnCharacter` place the respawn, so a corpse falling through a disappearing floor never matters) — but `endRun()` still destroys the arena floor under a live, undamaged character, and Roblox's gravity (~200 studs/s²) drops them fast enough to trip a leash range within under a second. The `Archer` spec hit this: identical failure twice, vanished the moment it re-grounded the player on `SpawnLocation` before testing. Any future spec placing something relative to the player's position after Phase10_Boss should do the same.
