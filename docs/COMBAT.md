...
**Light attack** — windup → active → recovery. Damage applies during the active window only.

**Dodge (Shift)** — costs stamina → short movement burst → invulnerability window → cooldown. The i-frame window is server-tracked; a client claiming invulnerability is ignored.
